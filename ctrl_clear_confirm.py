...
class clear_confirm_window(QMainWindow, Ui_clear_confirm):
    ...
    def on_confirm_btn_clicked(self):
        if var.clear_flag == 1:
            ...
            cur.close()
            conn.close()
            QMessageBox.question(self, '删除成功！', '装备数据已全部删除！', QMessageBox.Yes)
        elif var.clear_flag == 2:
            conn = sqlite3.connect("material_management.db")
            conn.text_factory = str
            cur = conn.cursor()
            ...
            cur.close()
            conn.close()
            QMessageBox.question(self, '删除成功！', '调入数据已全部删除！', QMessageBox.Yes)
            ...
            cur.close()
            conn.close()
            QMessageBox.question(self, '删除成功！', '调出数据已全部删除！', QMessageBox.Yes)
        else:
