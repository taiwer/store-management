...
class material_in_log_window(QMainWindow, Ui_material_in_out):
    ...
    def fill_table(self, datas):  # datas为列表
        ...
        self.model.setHorizontalHeaderLabels(['存货编码', '存货名称', '规格型号', '部署位置', '结存数量'])
        if datas != []:
            for i in range(0, len(datas)):
                ...
                item = QStandardItem(format(float(str(datas[i][4])), '.f'))
                item.setFlags(Qt.ItemIsSelectable | Qt.ItemIsEnabled)
    ...
    def init_table(self):
        ...
        conn.close()
        self.model = QStandardItemModel(len(datas), 5)
        self.model.setHorizontalHeaderLabels(['装备编码', '装备名称', '规格型号', '部署位置', '结存数量'])
        if datas != []:
            for i in range(0, len(datas)):
                item = QStandardItem(datas[i][0])
                item.setFlags(Qt.ItemIsSelectable | Qt.ItemIsEnabled)
                ...
                item = QStandardItem(format(float(str(datas[i][4])), '.0f'))
                item.setFlags(Qt.ItemIsSelectable | Qt.ItemIsEnabled)
                self.model.setItem(i, 4, item)
    ...
    def on_confirm_btn_clicked(self):
        ...
        if agree_man == "":
            QMessageBox.question(self, '操作失败！', '责任单位不可为空！', QMessageBox.Yes)
            return
        try:
            num_value = float(num)
        except:
        ...
        if log_type == "调出" and before_num < num_value:
            QMessageBox.question(self, '调出失败！', '装备数量不足！', QMessageBox.Yes)
            return
        ...
        if log_type == "调入":
            QMessageBox.question(self, '调入成功！', '装备成功调入！', QMessageBox.Yes)
        else:
            QMessageBox.question(self, '调出成功！', '装备成功调出！', QMessageBox.Yes)
    ...
    def keyPressEvent(self, event):
        if str(event.key()) == '16777220' or str(event.key()) == '16777221':
            if self.material_name_lineEdit.text() == "" and self.material_id_lineEdit.text() == "" and self.spec_lineEdit.text() == "":
                QMessageBox.question(self, '查询失败！', '请输入装备编码或装备名称或规格型号！', QMessageBox.Yes)
                self.fill_table([])
                return
            conn = sqlite3.connect("material_management.db")
            conn.text_factory = str
            ...
            conn.close()
            if res == []:
                QMessageBox.question(self, '查询失败！', '未找到相关装备！', QMessageBox.Yes)
                self.fill_table([])
                return
            self.fill_table(res)
            # 清空、重新加载数据
