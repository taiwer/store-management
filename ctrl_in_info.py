...
class in_info_window(QMainWindow, Ui_in_info):
    ...
    def on_export_btn_clicked(self):
        if self.export_data == []:
            QMessageBox.question(self, '导出失败！', '请确认表中存在调入数据！', QMessageBox.Yes)
            return
        book = Workbook()
        sheet = book.active
        sheet.append(('日期', '装备编码', '装备名称', '规格型号', '收入数量', '配发单位', '部署位置', '使用单位', '责任人', '责任单位'))
        ...
        tmp_dialog = QtWidgets.QFileDialog(None, "选取调入数据保存路径", "C:/")
        ...
        try:
            ...
            QMessageBox.question(self, '导出成功！', '调入数据已导出至Excel！', QMessageBox.Yes)
        except:
    ...
    def fill_table(self, datas):  # datas为列表
        ...
        self.model.setHorizontalHeaderLabels(
            ['日期', '装备编码', '装备名称', '规格型号', '收入数量', '配发单位', '部署位置', '使用单位', '责任人', '责任单位'])
        if datas != []:
            for i in range(0, len(datas)):
                ...
                item = QStandardItem('%.0f' % datas[i][4])
                item.setFlags(Qt.ItemIsSelectable | Qt.ItemIsEnabled)
                ...
                item.setFlags(Qt.ItemIsSelectable | Qt.ItemIsEnabled)
                self.model.setItem(i, 6, item) # 部署位置

                item = QStandardItem(str(datas[i][7]))
                item.setFlags(Qt.ItemIsSelectable | Qt.ItemIsEnabled)
                self.model.setItem(i, 7, item) # 使用单位
    ...
    def on_query_btn_clicked(self):
        ...
        if flag == 0:
            QMessageBox.question(self, '查询失败！', '调入数量范围应输入实数！', QMessageBox.Yes)
            self.fill_table([])
            return
        elif flag == 1:
            QMessageBox.question(self, '查询失败！', '调入数量范围冲突！', QMessageBox.Yes)
            self.fill_table([])
            return
        elif flag == 2:
            QMessageBox.question(self, '查询失败！', '调入日期范围冲突！', QMessageBox.Yes)
            self.fill_table([])
            return
        elif flag == 3:
            QMessageBox.question(self, '查询失败！', '未查询到相关调入信息！', QMessageBox.Yes)
            self.fill_table([])
            return
        else:
            self.fill_table(res)
            QMessageBox.question(self, '查询成功！', '相关调入记录已列在表中！', QMessageBox.Yes)
    ...
    def on_del_btn_clicked(self):
        ...
        for t in tt:
            indexs.append(t)
        if indexs == []:
            QMessageBox.question(self, '删除失败！', '请正确选择调入记录！', QMessageBox.Yes)
            return
        id_ll = []
        num_ll = []
        material_id_ll = []
        ...
        for i in range(0, len(id_ll)):
            ...
            if new_num < 0:
                if len(id_ll) == 1:
                    QMessageBox.question(self, '删除失败！', '若删除该条调入记录，对应结存数量小于0！\n未执行删除！', QMessageBox.Yes)
                else:
                    QMessageBox.question(self, '删除出错！', '成功删除' + str(i) + '条调入记录！\n' + '下一条调入记录若删除，对应结存数量小于0，停止删除！',
                                         QMessageBox.Yes)
                cur.close()
                conn.close()
                (flag, res) = self.my_query()
        ...
        cur.close()
        conn.close()
        QMessageBox.question(self, '删除成功！', str(len(indexs)) + '条调入记录已删除', QMessageBox.Yes)
        (flag, res) = self.my_query()
        self.fill_table(res)
        self.selected_row = -1
    ...
    def on_update_btn_clicked(self):
        ...
        for t in tt:
            indexs.append(t)
        if indexs == []:
            QMessageBox.question(self, '修改失败！', '请正确选择调入记录！', QMessageBox.Yes)
            return
        if len(indexs) > 1:
