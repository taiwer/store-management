...
class lots_new_window(QMainWindow, Ui_lots_new):
    ...
    def on_confirm_btn_clicked(self):
        if self.export_data == []:
            QMessageBox.question(self, '操作失败！', '批量新增装备详单中没有数据！', QMessageBox.Yes)
            return
        ...
        cur.execute(sql)
        conn.commit()
        QMessageBox.question(self, '批量新增装备成功！' , '批量操作完成！', QMessageBox.Yes)
    ...
    def fill_table(self, datas):  # datas为列表
        ...
        self.model.setHorizontalHeaderLabels(
            ['装备编码', '装备名称', '规格型号', '计量单位', '配发单位','部署位置','期初数量'])
        if datas != []:
            for i in range(0, len(datas)):
                ...
                item = QStandardItem(datas[i][5]) # 单价
                item.setFlags(Qt.ItemIsSelectable | Qt.ItemIsEnabled)
                self.model.setItem(i, 4, item)

                item = QStandardItem(datas[i][6]) # 部署位置
                item.setFlags(Qt.ItemIsSelectable | Qt.ItemIsEnabled)
                self.model.setItem(i, 5, item)

                item = QStandardItem(format(float(datas[i][4]), '.0f')) # 期初数量
                item.setFlags(Qt.ItemIsSelectable | Qt.ItemIsEnabled)
    ...
    def on_generate_btn_clicked(self):
        ...
        for i in range(0, id_len):
            ...
            if id_ll[i]=='':
                QMessageBox.question(self, '生成失败！', f'第{i+1}条装备编码为空！', QMessageBox.Yes)
                return
        ...
        if name_str == "":
            ...
            if len(name_ll) != id_len:
                QMessageBox.question(self, '生成失败！', '装备名称列表条数与装备编码列表不一致！', QMessageBox.Yes)
                return
            for i in range(0, id_len):
                name_ll[i] = name_ll[i].strip('\n\t ')
                if name_ll[i] == '':
                    QMessageBox.question(self, '生成失败！', f'第{i + 1}条装备名称为空！', QMessageBox.Yes)
                    return
        ...
        if spec_str == "":
            ...
            if len(spec_ll) != id_len:
                QMessageBox.question(self, '生成失败！', '规格型号列表条数与装备编码列表不一致！', QMessageBox.Yes)
                return
            for i in range(0, id_len):
        ...
        if unit_str == "":
            ...
            if len(unit_ll) != id_len:
                QMessageBox.question(self, '生成失败！', '计量单位列表条数与装备编码列表不一致！', QMessageBox.Yes)
                return
            for i in range(0, id_len):
        ...
        if ori_num_str == "":
            ...
            if len(ori_num_ll) != id_len:
                QMessageBox.question(self, '生成失败！', '期初数量列表条数与装备编码列表不一致！', QMessageBox.Yes)
                return
            for i in range(0, id_len):
        ...
        if pos_str == "":
            QMessageBox.question(self, '生成失败！', '部署位置不可为空！', QMessageBox.Yes)
            return
        else:
            ...
            if len(pos_ll) != id_len:
                QMessageBox.question(self, '生成失败！', '部署位置列表条数与装备编码列表不一致！', QMessageBox.Yes)
                return
            for i in range(0, id_len):
                pos_ll[i] = pos_ll[i].strip('\n\t ')
                if pos_ll[i] == '':
                    QMessageBox.question(self, '生成失败！', f'第{i + 1}条部署位置为空！', QMessageBox.Yes)
                    return

        price_str = self.price_textEdit.toPlainText()
        if price_str == "":
            QMessageBox.question(self, '生成失败！', '配发单位不可为空！', QMessageBox.Yes)
            return
        else:
            ...
            if len(price_ll) != id_len:
                QMessageBox.question(self, '生成失败！', '配发单位列表条数与装备编码列表不一致！', QMessageBox.Yes)
                return
            for i in range(0, id_len):
                price_ll[i] = price_ll[i].strip('\n\t ')
                if price_ll[i] == '':
                    QMessageBox.question(self, '生成失败！', f'第{i + 1}条配发单位为空！', QMessageBox.Yes)
                    return
        ...
        for i in range(len(id_ll)):
            if id_ll[i] in exist_id:
                QMessageBox.question(self, '生成失败！', '第' + str(i + 1) + "条装备编码重复，已在库中！", QMessageBox.Yes)
                cur.close()
                conn.close()
                return
        ...
        QMessageBox.question(self, '生成成功！', "批量新增装备详单已填充至下方表格，请仔细核查！", QMessageBox.Yes)
