...
class update_material_window(QMainWindow, Ui_update_material):
    ...
    def on_both_btn_clicked(self):
        ...
        id = id.strip("\n\t ")
        if id == "":
            QMessageBox.question(self, '更新失败！', '装备编码不可为空！', QMessageBox.Yes)
            return
        name = self.material_name_lineEdit.text()
        name = name.strip("\n\t ")
        if name == "":
            QMessageBox.question(self, '更新失败！', '装备名称不可为空！', QMessageBox.Yes)
            return
        spec = self.spec_lineEdit.text()
        spec = spec.strip("\n\t ")
        if spec == "":
        ...
        try:
            ...
            QMessageBox.question(self, '更新失败！', '新装备编码与已有装备重复！', QMessageBox.Yes)
            cur.close()
            conn.close()
            return
        ...
        cur.close()
        conn.close()
        QMessageBox.question(self, '更新成功！', '装备及对应调出/入信息已修改！', QMessageBox.Yes)
        self.close()
