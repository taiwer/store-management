...
class new_material_window(QMainWindow, Ui_new_material):
    def on_confirm_btn_clicked(self):
        ...
        if material_id == "":
            QMessageBox.question(self, '新增失败！', '装备编码不可为空！', QMessageBox.Yes)
            return
        material_name = self.material_name_lineEdit.text()
        material_name = material_name.strip("\n \t")
        if material_name == "":
            QMessageBox.question(self, '新增失败！', '装备名称不可为空！', QMessageBox.Yes)
            return
        ...
        if per_price == "":
            QMessageBox.question(self, '新增失败！', '配发单位不可为空！', QMessageBox.Yes)
            return
        ...
        now_num = ori_num
        tmp_material = Material(material_id, material_name, spec, unit, per_price, position, str(ori_num),
                                str(now_num))
        try:
            tmp_material.insert_new_material()
            QMessageBox.question(self, '新增成功！', '新装备信息已录入！', QMessageBox.Yes)
            ...
            QMessageBox.question(self, '新增失败！', '重复，装备库中已存在该装备编码！', QMessageBox.Yes)
