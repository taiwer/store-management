...
class import_confirm_window(QMainWindow, Ui_import_confirm):
    def __init__(self):
        ...
        if var.import_flag == 1:
            self.label.setText("警告！导入前将删除全部现有装备数据！")
            self.label_2.setText("该操作将删除现有装备数据！")
        elif var.import_flag == 2:
            self.label.setText("警告！导入前将删除全部现有调入记录！")
            self.label_2.setText("该操作将删除现有调入记录！")
        else:
            self.label.setText("警告！导入前将删除全部现有调出记录！")
            self.label_2.setText("该操作将删除现有调出记录！")
            ...
            if var.import_flag == 1:
                ...
                for i in range(0, len(all_ll)):
                    sql += "('" + all_ll[i][0] + "','" + all_ll[i][1] + "','" + all_ll[i][2] + "','" + all_ll[i][
                        3] + "','" + str(all_ll[i][4]) + "','" + all_ll[i][5] + "'," + all_ll[i][6] + "," + all_ll[i][7] + ")"
                    if i < len(all_ll) - 1:
                        sql += ","
                # for i in range(0, len(all_ll)):
                #     sql += "','".join(all_ll[0])
                ...
                cur.close()
                conn.close()
                QMessageBox.question(self, '导入存货成功！', '装备数据已全部导入装备数据库！', QMessageBox.Yes)
                ...
                conn.commit()
                cur.execute(
                    "create table in_log(date_time int,material_id varchar(32),material_name varchar(100),spec varchar(100),in_num float, per_price varchar(20), pos varchar(100), total_price varchar(100),user_man varchar(20),agree_man varchar(20), in_log_id INTEGER PRIMARY KEY AUTOINCREMENT)")
                ...
                cur.close()
                conn.close()
                QMessageBox.question(self, '导入调入记录成功！', '调入记录已全部导入装备调入数据库！', QMessageBox.Yes)
                ...
                cur.close()
                conn.close()
                QMessageBox.question(self, '导入调出记录成功！', '调出记录已全部导入装备调出数据库！', QMessageBox.Yes)
        except:
