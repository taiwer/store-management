...
class update_in_log_window(QMainWindow, Ui_update_in_log):
    ...
    def on_comfirm_btn_clicked(self):
        self.update_log()
        QMessageBox.question(self, '修改成功！', '1条调入记录已更新！', QMessageBox.Yes)
        self.close()
    ...
    def on_both_btn_clicked(self):
        ...
        cur.close()
        conn.close()
        QMessageBox.question(self, '修改成功！', '调入记录及对应装备的库存余量同时修改！', QMessageBox.Yes)
        self.close()
    ...
    def update_log(self):
        ...
        if agree_man == "":
            QMessageBox.question(self, '修改失败！', '责任单位不可为空！', QMessageBox.Yes)
            return 0
        ...
        if self.comboBox.currentIndex() == 0:  # 还是入库
            ...
            t_out_log = Out_Log(log_date, id, name, spec, num, per_price, position, per_price, user_man, agree_man)
