...
class update_out_log_window(QMainWindow, Ui_update_out_log):
    ...
    def on_comfirm_btn_clicked(self):
        self.update_log()
        QMessageBox.question(self, '修改成功！', '1条出库记录已更新！', QMessageBox.Yes)
        self.close()
    ...
    def on_both_btn_clicked(self):
        ...
        tid = tid.strip("\n\t ")
        if self.comboBox.currentIndex() == 1:  # 还是了出库
            self.now_num *= -1

        incr = self.now_num - self.ori_out_num
        conn = sqlite3.connect("material_management.db")
        conn.text_factory = str
        cur = conn.cursor()
        ...
        cur.close()
        conn.close()
        QMessageBox.question(self, '修改成功！', '调出记录及对应装备库存余量同时修改！', QMessageBox.Yes)
        self.close()
    ...
    def update_log(self):
        ...
        if self.comboBox.currentIndex() == 1:  # 还是出库
            conn = sqlite3.connect("material_management.db")
            conn.text_factory = str
            cur = conn.cursor()
            sql = "update out_log set date_time=" + log_date + ",out_num=" + num + ", per_price=" + per_price + ", pos='" + position + "', total_price=" + per_price + ", user_man='" + user_man + "', agree_man='" + agree_man + "' where out_log_id=" + self.ori_out_log_id
            cur.execute(sql)
            conn.commit()
            cur.close()
            conn.close()
            ...
            conn.text_factory = str
            cur = conn.cursor()
            sql = "delete from out_log where out_log_id=" + self.ori_out_log_id
            cur.execute(sql)
            conn.commit()
            cur.close()
            conn.close()

            t_in_log = In_Log(log_date, id, name, spec, num, per_price, position, per_price, user_man, agree_man)
            t_in_log.new_in_log()
        return 1
